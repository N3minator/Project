...
# Setting up colorlog for colored log output
...
# Dictionary for storing users' time zones (username -> offset in seconds)
user_timezones = {}
...
def format_time(dt, username=None):
    ...
    try:
        ...
        logger.error('Error getting time for user "%s": %s', username, e)
        return "Unknown time"
...
def broadcast(message, exclude_client=None):
    ...
    If a client disconnects during sending, removes it from the list and notifies others.
    """
...
def broadcast_online_users():
    """
    Sends a system message with information about the current number of connected users.
    """
    ...
    for username in user_list:
        msg += "|" + username
    logger.debug("Broadcasting online list: %s", msg)
    broadcast(msg)
...
def handle_client(client_socket):
    ...
    username = None
    try:
        # Read data for user identification (name and offset)
        ...
        # System message about joining (NOT saved to DB)
        ...
        logger.info("User '%s' entered the chat.", username)
        ...
        while True:
            ...
            if message_text.startswith("/pm "):
                ...
                if target_socket:
                    try:
                        ...
                        session_db.rollback()
                        client_socket.sendall("Error saving private message to the DB.\n".encode('utf-8'))
                        continue
                    ...
                    try:
                        ...
                        logger.error("Error sending private message to recipient '%s': %s", target, e)
            ...
            # Handling public messages (saving to DB)
            try:
                ...
                logger.error("Error saving public message '%s': %s", username, e, exc_info=True)
                session_db.rollback()
                client_socket.sendall("Error saving message to the DB.\n".encode('utf-8'))
                continue
        ...
        if client_socket in clients:
            ...
            # System message about disconnection (NOT saved to DB)
